...
class OdaienvViewResources  extends JViewLegacy {
	...
	protected function addToolbar()
	{
		...
		{
			JToolbarHelper::publish('resources.publish', 'JTOOLBAR_PUBLISH', true);
			JToolbarHelper::unpublish('resources.unpublish', 'JTOOLBAR_UNPUBLISH', true);
		}
		...
		if ($canDo->get('core.edit.state'))
		{
			if ($this->state->get('filter.state') != -1)
			{
				if ($this->state->get('filter.state') != 2)
				{
					JToolbarHelper::archiveList('resources.archive');
				}
		...
		if(!version_compare(JVERSION,'3','<')){		
			JHtmlSidebar::setAction('index.php?option=com_odaienv&view=resources');
		}
				if(!version_compare(JVERSION,'3','<')){
