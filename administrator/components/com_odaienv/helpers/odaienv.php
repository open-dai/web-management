...
class OdaienvHelper
{
	...
	public static function getActions($categoryId = 0)
	{
		...
		if (empty($categoryId))
		{
			$assetName = 'com_odaienv';
			$level = 'component';
		}
		else
		{
			$assetName = 'com_odaienv.category.'.(int) $categoryId;
			$level = 'category';
		}
	
		$actions = JAccess::getActions('com_odaienv', $level);
	...
	public static function getExtensions() 
	{
		...
		$xml = simplexml_load_file(JPATH_ADMINISTRATOR.'/components/com_odaienv/elements/extensions.xml', 'JXMLElement');		        
	...
	public static function getCategoryViews()
	{
		...
		foreach($extensions as $extension ) {
			$views[$extension->listview->__toString()] = 'com_odaienv.'.$extension->name->__toString();
		}
		return $views;
	}	
}
...
abstract class JHtmlOdaienv
{
	...
	public static function extensions($ext) 
	{
		...
		foreach ($extensions as $extension) {   
			...
			$option->value = 'com_odaienv.'.$extension->name;
			$options[] = $option;			
		}		
	...
	public static function categories($extension, $cat_id,$name="categories",$title="Select Category", $config = array('attributes'=>'class="inputbox"','filter.published' => array(0,1)))
	{
			...
			$query->from('#__odaienv_categories AS a');
